...
class Xfstesting(TestSuite):
    ...
    def xfstesting_xfs_standard_datadisk_validation(self, node: Node) -> None:
        ...
        config_path = xfstests.get_xfstests_config_path()
        node.execute(
            f"echo 'SCRATCH_DEV=/dev/sdc2' >> {config_path}", sudo=True, shell=True
        )
        node.execute(
            f"echo 'SCRATCH_MNT=/root/scratch' >> {config_path}", sudo=True, shell=True
        )
        node.execute(
            f"echo 'TEST_DEV=/dev/sdc1' >> {config_path}", sudo=True, shell=True
        )
        node.execute(
